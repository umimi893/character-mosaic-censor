...
class ImageCanvas(QWidget):
    ...
    def set_state(self, state: _CanvasState | None, show_boxes: bool, show_body_map: bool = False) -> None:
        ...
        self._show_body_map = show_body_map
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        if self._show_body_map:
            self._draw_body_map(painter, left, top, sx, sy, scale)
            if self._show_boxes:
                self._draw_candidate_evidence(painter, left, top, sx, sy, scale)
            return
        if not self._show_boxes:
            return

        pen = QPen(QColor(255, 171, 46, 215))
        pen.setWidthF(max(1.5, 2.0 * min(1.5, scale)))
        pen.setStyle(Qt.PenStyle.DashLine)
        painter.setPen(pen)
        for box in self._state.censor_boxes:
            painter.drawEllipse(self._map_box(box, left, top, sx, sy))

        pen = QPen(QColor(58, 224, 139, 245))
        pen.setWidthF(max(2.0, 2.5 * min(1.5, scale)))
        painter.setPen(pen)
        for det in self._state.detections:
            rect = self._map_box(det.box, left, top, sx, sy)
            painter.drawRect(rect)
            self._draw_label(painter, rect, f"{det.label} {det.score:.2f}", QColor("#3ae08b"))

    def _draw_body_map(self, painter, left, top, sx, sy, scale) -> None:
        if self._state is None:
            return
        pen = QPen(QColor(65, 190, 230, 180))
        pen.setWidthF(max(1.2, 1.8 * min(1.5, scale)))
        painter.setPen(pen)
        for edge in self._state.pose_edges:
            painter.drawLine(
                QPointF(left + edge.start[0] * sx, top + edge.start[1] * sy),
                QPointF(left + edge.end[0] * sx, top + edge.end[1] * sy),
            )
        for region in self._state.body_regions:
            color, dashed = _region_style(region.kind)
            pen = QPen(color)
            pen.setWidthF(max(1.2, 1.8 * min(1.5, scale)))
            if dashed:
                pen.setStyle(Qt.PenStyle.DashLine)
            painter.setPen(pen)
            rect = self._map_box(region.box, left, top, sx, sy)
            painter.drawRect(rect)
        ...
        painter.setPen(QPen(QColor(121, 213, 255, 230)))
        painter.setBrush(QColor(121, 213, 255, 180))
    ...
    def _draw_candidate_evidence(self, painter, left, top, sx, sy, scale) -> None:
        ...
        for index, evidence in enumerate(self._state.candidate_evidence):
            color = _decision_color(evidence.decision)
            pen = QPen(color)
            pen.setWidthF(max(2.0, (3.5 if index == self._selected_candidate else 2.4) * min(1.5, scale)))
            if evidence.decision == "suppress":
                pen.setStyle(Qt.PenStyle.DashLine)
            painter.setPen(pen)
            rect = self._map_box(evidence.detection.box, left, top, sx, sy)
            painter.drawRect(rect)
            self._candidate_rects.append((index, rect))
            self._draw_label(painter, rect, f"#{index + 1} {evidence.decision.upper()} {evidence.detection.score:.2f}", color)
    ...
    def mousePressEvent(self, event) -> None:  # noqa: N802
        ...
        for index, rect in reversed(self._candidate_rects):
            if rect.adjusted(-5, -5, 5, 5).contains(point):
                ...
                break
        if hit >= 0:
            self._selected_candidate = hit
            self.candidate_selected.emit(hit)
            self.update()
    ...
    def _draw_label(painter, rect, text, accent, compact=False) -> None:
        ...
        px = 5 if compact else 6
        py = 3 if compact else 4
        width, height = text_rect.width() + px * 2, text_rect.height() + py * 2
        x, y = rect.left(), max(0.0, rect.top() - height)
        painter.fillRect(QRectF(x, y, width, height), QColor(7, 14, 18, 225))
        painter.setPen(accent)
        painter.drawText(QPointF(x + px, y + height - py - 1), text)
...
class PreviewWidget(QWidget):
    ...
    def __init__(self, language: str = "ja", parent: QWidget | None = None):
        super().__init__(parent)
        self._language = normalize_language(language)
        self._states = {"original": None, "detected": None, "analysis": None, "censored": None}
        self._current_mode = "original"
        self._selected_candidate = -1

        self.canvas = ImageCanvas(self._language, self)
        self.canvas.candidate_selected.connect(self._on_candidate_selected)
        ...
        for button in (self.original_btn, self.detected_btn, self.analysis_btn, self.censored_btn):
    ...
    def set_language(self, language):
        ...
        self.analysis_btn.setText(self._t("人体解析", "Body analysis"))
        ...
        if not any(self._states.values()):
            ...
            self.evidence_label.setText(self._t("人体解析結果はここに表示されます", "Body-analysis evidence appears here"))
    ...
    def set_frame(self, frame: PreviewFrame) -> None:
        state = _CanvasState(
            _pil_to_qimage(frame.image), frame.detections, frame.censor_boxes, frame.status,
            frame.coordinate_size or frame.image.size, frame.body_regions, frame.pose_points,
            frame.pose_edges, frame.candidate_evidence, frame.analysis_status,
        )
        self.file_label.setText(frame.source.name)
        self.file_label.setToolTip(str(frame.source))

        if frame.stage == "original":
            self._states = {"original": state, "detected": None, "analysis": None, "censored": None}
            self._selected_candidate = -1
            self._activate_mode("original")
        elif frame.stage == "analysis":
            self._states["analysis"] = state
            self._selected_candidate = -1
            self._activate_mode("analysis")
        elif frame.stage in {"detecting", "detected"}:
            self._states["detected"] = state
            if frame.stage == "detected" and self._states["analysis"] is None and frame.candidate_evidence:
                self._states["analysis"] = state
            # Once body analysis is available, keep it visible long enough to
            # inspect the skeleton/regions instead of instantly switching away.
    ...
    def _activate_mode(self, mode):
        ...
        {"original": self.original_btn, "detected": self.detected_btn, "analysis": self.analysis_btn, "censored": self.censored_btn}[mode].setChecked(True)
    ...
    def _on_candidate_selected(self, index):
        self._selected_candidate = index
        self.canvas.set_selected_candidate(index)
        self._refresh_evidence()
    ...
    def _refresh(self):
        ...
        if state is None:
            for fallback in ("analysis", "censored", "detected", "original"):
                if self._states.get(fallback) is not None:
                    state = self._states[fallback]
                    break
        self.canvas.set_selected_candidate(self._selected_candidate)
        self.canvas.set_state(state, self.box_toggle.isChecked(), show_body_map=self._current_mode == "analysis")

    def _refresh_evidence(self):
        state = self._states.get("analysis")
        if state is None or not state.candidate_evidence:
            self.evidence_label.setText(
                self._t(f"人体解析: {state.analysis_status}", f"Body analysis: {state.analysis_status}")
                if state is not None and state.analysis_status
                else self._t("人体解析データなし", "No body-analysis data")
            )
            return
        if self._selected_candidate < 0 or self._selected_candidate >= len(state.candidate_evidence):
            keep = sum(e.decision == "keep" for e in state.candidate_evidence)
            review = sum(e.decision == "review" for e in state.candidate_evidence)
            suppress = sum(e.decision == "suppress" for e in state.candidate_evidence)
            self.evidence_label.setText(self._t(
                f"候補をクリックすると判定理由を表示します。KEEP {keep} / REVIEW {review} / SUPPRESS {suppress} / 状態 {state.analysis_status}",
                f"Click a candidate to inspect evidence. KEEP {keep} / REVIEW {review} / SUPPRESS {suppress} / status {state.analysis_status}",
            ))
            return
        evidence = state.candidate_evidence[self._selected_candidate]
        pos = " / ".join(_humanize_signal(s, self._language) for s in evidence.positive_signals) or "-"
        neg = " / ".join(_humanize_signal(s, self._language) for s in evidence.negative_signals) or "-"
        self.evidence_label.setText(self._t(
            f"候補 #{self._selected_candidate + 1}  判定: {evidence.decision.upper()}  |  保持材料: {pos}  |  除外材料: {neg}",
            f"Candidate #{self._selected_candidate + 1}  decision: {evidence.decision.upper()}  |  keep evidence: {pos}  |  negative evidence: {neg}",
        ))


def _decision_color(decision):
    if decision == "suppress":
        return QColor(244, 92, 92, 245)
    if decision == "review":
        return QColor(255, 184, 77, 245)
    return QColor(58, 224, 139, 245)


def _region_style(kind):
    if kind == "person":
        return QColor(93, 165, 255, 190), True
    if kind == "head":
        return QColor(190, 120, 255, 210), False
    if kind == "face":
        return QColor(255, 120, 205, 210), False
    if kind == "eye":
        return QColor(255, 230, 90, 230), False
    if kind == "pelvis_safe":
        return QColor(71, 232, 155, 220), True
    if "knee_zone" in kind:
        return QColor(255, 126, 76, 215), True
    if "armpit_zone" in kind:
        return QColor(255, 156, 86, 215), True
...
def _humanize_signal(signal, language):
    if language == "en":
        return signal
    mapping = {
        "detector": "検出信頼度", "near_pelvis": "骨盤付近", "inside_head": "頭BBox内",
        "inside_face": "顔BBox内", "inside_eye": "目BBox内", "near_right_knee": "右膝付近",
        "near_left_knee": "左膝付近", "near_right_armpit": "右腋付近", "near_left_armpit": "左腋付近",
    }
    prefix, sep, rest = signal.partition(":")
    label = mapping.get(prefix, prefix)
    return f"{label}:{rest}" if sep else label
