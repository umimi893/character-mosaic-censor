...
- 日本語 / Englishの表示言語切替を追加し、選択を次回起動用に保存。
- 詳細設定を「基本」「大きな画像」「重複検出の調整」の3タブに再編し、各設定の影響を表示。
- `run.bat` を `pythonw.exe` の分離起動に変更し、GUI起動後にCMD画面を残さない。
- 画像内の想定人数を指定でき、検出数と不一致の画像を `_manual_review` へ自動隔離。
- 手修正用の原本と、検出位置を描画した確認画像を分けて保存。
- 四角いBBox全体ではなく、アンチエイリアスされた楕円マスクでモザイク・ブラー・黒塗りを適用。
- 通常検出が0件の場合だけ、左右/上下反転と90/180/270度回転で再検出。
- 入力だけ指定した場合は `_censored` 出力先を自動設定。
- ONNX Runtime GPUをCUDA 12.8互換の1.26.0に固定。
- 回帰テストを48件へ拡張。
